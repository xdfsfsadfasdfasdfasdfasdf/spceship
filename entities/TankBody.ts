...
import type { CameraEntity } from "../src/Native/Camera";

import AbstractShape from "./AbstractShape";
import NecromancerSquare from "./NecromancerSquare";
import LivingEntity from "./Live";
import ObjectEntity from "./Object";
import Barrel from "./Barrel";
import AutoTurret from "./AutoTurret";

import { Color, StyleFlags, StatCount, Tank, CameraFlags, Stat, InputFlags, PhysicsFlags, PositionFlags, NameFlags, HealthFlags, EntityTags } from "../src/Const/Enums";
import { Entity } from "../src/Native/Entity";
import { NameGroup, ScoreGroup } from "../src/Native/FieldGroups";
import { Addon, AddonById, GuardObject } from "./Addons";
import { getTankById, TankDefinition, BarrelDefinition, visibilityRateDamage } from "../src/Const/TankDefinitions";
import { DevTank } from "../src/Const/DevTankDefinitions";
import { Inputs } from "./AI";
import { ArenaState } from "../src/Native/Arena";
import { AccessLevel, maxPlayerLevel } from "../src/config";
...
export default class TankBody extends LivingEntity implements BarrelBase {
    ...
    public setTank(id: Tank | DevTank) {
        ...
        const hasSpin = Boolean(tank.spin && tank.spin > 0);

        let rotator: GuardObject | null = null;
        if (hasSpin) {
            const rotSpeed = tank.spin ? tank.spin * 0.02 : 0.01;
            rotator = new GuardObject(this.game, this, 1, 0.1, 0, rotSpeed);
            rotator.scaleFactor = this.scaleFactor;
            if (rotator.styleData.values.flags & StyleFlags.isVisible) {
                rotator.styleData.values.flags ^= StyleFlags.isVisible;
            }
        }

        for (const barrel of tank.barrels) {
            const parentObj = barrel.isAuto ? this : (rotator ?? this);
            if (barrel.isAuto) {
                ...
                const dist = barrel.distance ?? (this.baseSize * 0.8);
                const rad = barrel.angle;
                const scale = this.scaleFactor;
                const x = Math.cos(rad) * dist * scale - Math.sin(rad) * (barrel.offset ?? 0) * scale;
                const y = Math.sin(rad) * dist * scale + Math.cos(rad) * (barrel.offset ?? 0) * scale;

                const autoTurret = new AutoTurret(parentObj, turretDef, Math.max(15, barrel.width * 0.6));
                ...
                this.barrels.push(new Barrel(parentObj, barrel));
            }
        }
