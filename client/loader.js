...
Module.todo.push([(dependency, servers, tanks) => {
    ...
    parser.addCodeElementParser(null, function({ index, bytes }) {
        ...
        switch(index) {
            ...
            // modify upgrade menu grid layout from 2 wide to 5 wide & double tile size
            case 160: {
                const arr = Array.from(bytes);
                for(let i = 0; i < arr.length - 3; i++) {
                    if(arr[i] === OP_I32_CONST && arr[i+1] === 2 && arr[i+2] === OP_SET_LOCAL && arr[i+3] === 4) {
                        arr[i+1] = 5;
                    }
                }
            ...
            case 203: {
                ...
                for(let i = 0; i < arr.length - 2; i++) {
                    if(arr[i] === OP_I32_CONST && arr[i+1] === 2 && arr[i+2] === OP_I32_SHR_U) {
                        arr[i+1] = 5;
                        arr[i+2] = OP_I32_DIV_U;
                    }
                }
                // 2. Double tile size from 63 to 126
                for(let i = 0; i < arr.length - 1; i++) {
                    if(arr[i] === OP_I32_CONST && arr[i+1] === 63) {
                        arr[i+1] = 126;
                    }
                }
                return new Uint8Array(arr);
...
Module.todo.push([() => {
    ...
    try {
        if (window.input && window.input.set_convar) {
            window.input.set_convar("ren_background_color", "0x0d0714");
            window.input.set_convar("ren_grid_color", "0x1a0d2e");
            window.input.set_convar("ren_border_color", "0xffffff");
        }
    } catch (e) {
        console.warn("Space theme convar initialization deferred:", e);
    }
...
class ASMConsts {
    ...
    static getFontsLoaded() {
        return document.fonts.check("1px 'Press Start 2P'") || document.fonts.check("1px Ubuntu");
    }

    static setContextFont(ctxId, fontSize) {
        Module.cp5.contexts[ctxId].font = Math.round(fontSize * 0.65) + "px 'Press Start 2P', Ubuntu, sans-serif";
    }
